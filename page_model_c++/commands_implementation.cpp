...
void error_exit(std::string error_text) {
	std::cout << error_text << std::endl;
	exit(1);
}

std::string get_appropriate_code_command(std::string command_codem, int &size_index, int shift) {
	if ((size_index + 2) < command_codem.size()) 
		size_index += shift;
	else
		error_exit("Error occured after reading string out of size border!");
		  
	return command_codem.substr(size_index-shift, size_index);
}

cmd_code examine_command_code(std::string whole_prog_str, int index) {
	cmd_code comm_code; 
	comm_code.cm_name = get_appropriate_code_command(whole_prog_str, index, 2);

	if(comm_code.cm_name == "1A" || comm_code.cm_name == "80") {
		comm_code.part_one = get_appropriate_code_command(whole_prog_str, index, 2);
		comm_code.cmd_types.push_back("reg");
		comm_code.part_two = get_appropriate_code_command(whole_prog_str, index, 2);
		comm_code.cmd_types.push_back("reg");
	}	
	else if (comm_code.cm_name == "1B") {
		comm_code.part_one = get_appropriate_code_command(whole_prog_str, index, 2);
		comm_code.cmd_types.push_back("reg");
		comm_code.part_two = get_appropriate_code_command(whole_prog_str, index, 8);
		comm_code.cmd_types.push_back("addr");
	}
	else if (comm_code.cm_name == "0C" || comm_code.cm_name == "21") {
		comm_code.part_one = get_appropriate_code_command(whole_prog_str, index, 2);
		comm_code.cmd_types.push_back("reg");
		comm_code.part_two = get_appropriate_code_command(whole_prog_str, index, 2);
		comm_code.cmd_types.push_back("reg");
		comm_code.part_three = get_appropriate_code_command(whole_prog_str, index, 2);
		comm_code.cmd_types.push_back("reg");
	}
	else if (comm_code.cm_name == "0D" || comm_code.cm_name == "23") {
		comm_code.part_one = get_appropriate_code_command(whole_prog_str, index, 2);
		comm_code.cmd_types.push_back("reg");
		comm_code.part_two = get_appropriate_code_command(whole_prog_str, index, 2);
		comm_code.cmd_types.push_back("reg");
		comm_code.part_three = get_appropriate_code_command(whole_prog_str, index, 8);
		comm_code.cmd_types.push_back("addr");
	}
	else if (comm_code.cm_name == "1C") {
		comm_code.part_one = get_appropriate_code_command(whole_prog_str, index, 2);
		comm_code.cmd_types.push_back("reg");
		if (comm_code.cm_name[0] == '0') {
			comm_code.part_two = get_appropriate_code_command(whole_prog_str, index, 2);
			comm_code.cmd_types.push_back("lit8");
		}
		else if (comm_code.cm_name[1] == '1') {
			comm_code.part_two = get_appropriate_code_command(whole_prog_str, index, 4);
			comm_code.cmd_types.push_back("lit16");
		}
		else {
			//error output
		}
...
bool form_command_text(std::string whole_text) {

cmd_code command_code_;
if (!command_code_.cm_name.empty()) {
	
	if (!command_code_.part_one.empty()) {

		if (!command_code_.part_two.empty()) {

			if (!command_code_.part_three.empty()) {
